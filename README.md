...
| 指标 | 优化前 | 优化后（测试集内） | 泛化验证(v4) |
|------|--------|------------------|-------------|
| 意图识别准确率 | **37.0%** | **100.0%** | **96.7%** |
...
**泛化率提升路径**: 48% → 60% → 96.7%（通过分析错误案例补充规则 + 动态置信度 + 复合意图）

> ⚠️ **说明**: v4泛化测试集为自然对话风格（15-25字符/条），共30条样本，更接近真实用户表达习惯。
...
| 指标 | 测试集内 | 第一轮Held-out | 第二轮Held-out | v4 Held-out（自然对话） |
|------|---------|---------------|---------------|---------------------|
| 样本数 | 200 | 50 | 30 | 30 |
| 准确率 | **100.0%** | **48.0%** | **60.0%** | **96.7%** |

**v4测试集特点**: 自然对话风格（15-25字符/条），包含口语化表达，更接近真实用户场景。
...
### 🏗️ 架构升级：动态置信度 + 反问确认 + 复合意图

#### 动态置信度计算
- 根据匹配模式长度和匹配比例动态计算置信度
- 精确长模式(>0.9)、短模式(0.55-0.85)、通配模式(≤0.75)
- 匹配文本占比越高，置信度越高

#### 反问确认机制
- 低置信度匹配(0.35-0.55)时返回`ask_for_clarification`意图
- 自动生成反问句："请问您是想{意图描述}吗？"
- 避免低质量匹配导致的错误回复

#### 复合意图Pipeline
- 支持多意图识别："又甜又贵" → `[complaint_taste, complaint_price]`
- 识别流程：规则匹配 → 复合意图检测 → 子意图路由
- 当前支持4种复合模式：口感+价格、份量+口感、推荐+价格、下单+优惠
...
> - 真实生产环境预期准确率：约85-90%（规则覆盖+动态置信度+LLM兜底）
